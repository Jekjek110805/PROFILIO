...
interface Props {
    ...
    /**
     * Visual treatment for the card behind each image — background, blur,
     * border. Needed for transparent logo art. Overrides the default shadow.
     */
    cardStyle?: React.CSSProperties;
    /** Inset between the card edge and the image, in px. Scales with the card. */
    cardPadding?: number;
...
function resolveAlt(item: unknown): string {
    const image = (item as ImageInput)?.image;
...
export default function EyeTicker({
    ...
    // Scale the inset with the card, or narrow viewports leave no room for art.
    const padding = cardPadding == null ? undefined : cardPadding * scale;
    ...
    const topKey = JSON.stringify(topImages);
    const bottomKey = JSON.stringify(bottomImages);
